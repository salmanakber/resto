...
export const generateReceiptHTML = (order: OrderReceiptProps['order'], currency: [string, string, string] | null, company: any, taxSettings: TaxSettings) => {
  const currencySymbol = currency?.[1] || '$';
  ...
  const calculateTax = (amount: number) => {
    ...
    return { gst, pst, hst, totalTax, totalAmount: amount + totalTax, grossAmount: amount };
  };
  ...
  return `
    ...
    <html>
      ...
      <body>
        <div class="header">
          <h1>${company ? company.name : 'PICKUP ORDER RECEIPT'}</h1>
        ...
        <div class="section">
          ...
          ${order.orderType === 'dine-in' ? `   
            <p>${JSON.parse(order.dineInCustomer).name}</p>
            <p>${JSON.parse(order.dineInCustomer).phone}</p>
          ` : `
            <p>${order.customerDetails ? JSON.parse(order.customerDetails).name : ''}</p>
            <p>${order.customerDetails ? JSON.parse(order.customerDetails).phone : ''}</p>
            <p>${order.customerDetails ? JSON.parse(order.customerDetails).email : ''}</p>
          `}
        </div>
          ...
          <div class="items">
              ...
              <div class="item">
                <div class="flex-between">
                  ...
                  <span>${formatMoney( (item.quantity) ? item.price * item.quantity : item.price)}</span>
                </div>
                ${item.selectedAddons?.map(addon => `
                  <div class="item-addon flex-between">
                    <span>+ ${addon.name} x${(addon.quantity) ? addon.quantity : 1}</span>
                    <span>${formatMoney( (addon.quantity) ? addon.price * addon.quantity : addon.price)}</span>
                  </div>
                `).join('') || ''}
              </div>
            `).join('')}
        ...
        <div class="total">
          <div class="flex-between">
            <span>Subtotal</span>
            <span>${ (order.totalAmount === tax.grossAmount) ? formatMoney(Number(order.totalAmount) - tax.totalTax) : formatMoney(Number(order.totalAmount))}</span>
          </div>
          ...
          <div class="flex-between font-bold mt-1">
            <span>Total</span>
            <span>${formatMoney(tax.grossAmount)}</span>
          </div>
        </div>
        ...
        <div class="qr-code">
          ...
          <p>
          ${order.orderType === 'pickup' ? `OTP: ${order.otp}` : ''}
          </p>
        </div>
...
export default function OrderReceipt({ order, currency, company, taxSettings }: OrderReceiptProps) {
